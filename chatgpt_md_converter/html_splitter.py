...
def split_html_for_telegram(text: str) -> list[str]:
    ...
    for part in parts:
        ...
        if part.startswith("<pre>") or part.startswith("<pre><code"):
            ...
            # обычный HTML разбиваем по логике
            ...
            if current.strip():
                ...
                close_tags = tracker.get_closing_tags_html()
                chunks.append(open_tags + current + close_tags)
    # постобъединение: склеиваем чанки, если в сумме они не превышают лимит
    merged_chunks = []
    buf = ""
    for chunk in chunks:
        # chunk = chunk.lstrip("\n")  # удаляем ведущие переносы строки
